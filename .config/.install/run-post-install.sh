...
# are auto-skipped on Omarchy (fonts install, Garuda icons/themes removal,
# GTK theme, waybar k10temp). The install runs from a live
...
if [[ -f $POST_INSTALL ]]; then
    ...
    info "are auto-skipped here; skip steps that don't apply to this machine)."
    bash "$POST_INSTALL"
else
