...
BarWidget {
  id: root
  moduleName: "taeryn.tray"

  // Click-toggle drawer, persisted in the shell.json entry via the same
  // updateEntryInline path as pin/hide (stock Tray is hover-driven, which a
  // touchscreen can only flash open for the duration of a press).
  readonly property bool expanded: setting("expanded", false) === true

  function toggleDrawer() {
    if (!root.bar || !root.bar.shell || typeof root.bar.shell.updateEntryInline !== "function") return
    var id = root.moduleName || "taeryn.tray"
    root.bar.shell.updateEntryInline(id, { id: id, expanded: !expanded })
  }
