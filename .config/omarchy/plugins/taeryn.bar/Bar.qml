...
Item {
  ...
  // taeryn: NOT `required` — shell.qml's pluginBarLoader loads custom bars via
  // Loader.source, which cannot pass creation-time properties, so a required
  // prop fails instantiation before configureBar() runs (upstream bug: the
  // fallback path itself crashes on Loader.errorString, which Loaders don't
  // have). Plain defaults let the loader create us; configureBar() injects
  // the real values immediately after.
  ...
  component BarPanel: PanelWindow {
    ...
    color: root.transparent ? Qt.rgba(root.background.r, root.background.g, root.background.b, 0.55) : root.background
  ...
  component ModuleSlot: Item {
    ...
    MouseArea {
      ...
      property real pressedX: 0
      property real pressedY: 0
      readonly property bool canReorder: false  // taeryn: drag-reorder disabled — touchscreen taps kept becoming accidental drags
      readonly property real dragThreshold: Style.space(4)
