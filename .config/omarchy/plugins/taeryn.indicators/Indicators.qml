...
BarWidget {
  ...
  // Drawer collapse: everything (active + inactive) hides behind the chevron
  // until it is clicked open. Persisted in the shell.json entry like Tray's
  ...
  function persistDrawerState(collapsed) {
    if (!bar || !bar.shell || typeof bar.shell.updateEntryInline !== "function") return
    var id = moduleName || "taeryn.indicators"
    bar.shell.updateEntryInline(id, { id: id, collapsed: collapsed })
  }

  function toggleDrawer() {
    persistDrawerState(!drawerCollapsed)
  }
  ...
  Row {
    ...
    WidgetButton {
      id: drawerChevronH
      bar: root.bar
      // Collapsed: chevron-right points at the hidden content; expanded: chevron-left.
